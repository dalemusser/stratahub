...
| **Unit 1 – Unit 5** | One dot per unit. The dot's color shows whether the unit is downloaded on this device. A green ring around the dot marks the student's current unit, and a green check mark beside the dot marks a unit they have completed. An **amber** cell means the student's current unit is not on this device. See *Reading the unit dots* below. |
...
Each unit cell shows two things at once. The **dot** is about the device. The **ring or check mark** is about the student. The legend above the table says the same thing in short form: *Dot: on this device. Ring and check: student's progress.*
...
| Mark | Meaning |
|---|---|
| **Green ring** around the dot | **Current unit.** The first unit the student has not yet completed. This is normally the unit they are playing now. A blue dot inside the ring means the device is ready. |
| **Amber ring and dot** | **Current unit not on this device.** The unit the student is playing is not downloaded on this device. This is the cell to look for. |
| **Green check** beside the dot | **Completed.** The student has passed every progress point in this unit. |

**Amber is the only color that means "do something."** Scan the table for amber cells. Each one is a student who will wait for a download, or fail to launch, when they open MHS on that device. Blue and green mean ready; gray on a unit the student is not playing yet means nothing needs attention today.

**A check over a faded gray dot is normal.** MHS removes finished units from the device to save space, so completed units almost always show as gray with a check. That is not a problem.

Two things to keep in mind when reading the cells:

**1. The ring and check come from gameplay, not from the device.** They show the student's progress, which is saved online with their account. When a student switches devices, the ring appears on the new device right away, in amber, until the unit downloads there. MHS starts that download on its own when the student opens it, and the student's own screen says **Ready to play** once it has finished.
...
> **Tip**
> Green check marks are a quick way to count completed units for the whole class. For where a student is *within* a unit, and for flagged tasks, use the Progress view.
...
2. Scan for **amber** cells. Each one is a student whose current unit is not on their device yet. If the class is moving to a new unit today, also look down that unit's column for gray dots. A download starts on its own when the student opens MHS, and takes a few minutes on school Wi-Fi.
...
> **Tip**
> If many cells are amber, have those students open Mission HydroSci as they arrive so the downloads run while you start class, instead of everyone downloading at the same moment when you say "launch."

## Solving problems with the Devices view

| The student says… | Look at… | What it usually means | What to do |
|---|---|---|---|
| "It's stuck on the loading screen" or "It won't launch." | Their current unit's cell, and Storage. | Amber: the unit is not on this device. Blue outline inside the ring: it is still downloading. | Have the student stay on the Mission HydroSci page until it says **Ready to play**. MHS retries a stopped download on its own; the page shows what it is doing and when it will retry. If the bar is red, free up space first. |
...
| "I'm on a different Chromebook today." | A new row under their name, amber until the unit downloads there. | Nothing is wrong. The new device downloads its own copy of the unit; progress follows the account. | Have them open MHS a few minutes early so the download finishes before you start. |
...
The Devices view gives a unit-level snapshot: green check marks are units the student has completed, and the green ring marks the unit they are working in. That is enough for a quick "who is on which unit" scan of the class.
...
|---|---|
| Whether a student's device is ready for the unit they are playing | The ring: green is ready, amber means the unit is not on this device |
...
| Which units a student has completed | Green check marks |
