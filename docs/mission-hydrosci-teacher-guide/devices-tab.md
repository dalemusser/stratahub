...
| **Unit 1 – Unit 5** | One dot per unit. The dot's color shows whether the unit is downloaded on this device. A green ring around the dot marks the student's current unit, and a green check mark beside the dot marks a unit they have completed. See *Reading the unit dots* below. |
...
Each unit cell shows two things at once. The **dot** is about the device. The **ring or check mark** is about the student.
...
| Mark | Meaning |
|---|---|
| **Green ring** around the dot | **Current unit.** The first unit the student has not yet completed. This is normally the unit they are playing now. |
| **Green check** beside the dot | **Completed.** The student has passed every progress point in this unit. |

The cell to look for is a **gray dot inside a green ring**. It means the unit the student is playing is not downloaded on this device, so they will wait for a download when they open MHS. A blue dot inside the ring means they are ready to go.

Two things to keep in mind when reading the cells:

**1. The ring and check come from gameplay, not from the device.** They show the student's progress, which is saved online with their account. When a student switches devices, the ring appears on the new device right away, with a gray dot inside it until the unit downloads there. MHS starts that download on its own when the student opens it, and the student's own screen says **Ready to play** once it has finished.
...
2. Find each student's green ring and look at the dot inside it. **Gray** means the unit they are playing is not on that device yet. If the class is moving to a new unit today, also look down that unit's column for gray dots. A download starts on its own when the student opens MHS, and takes a few minutes on school Wi-Fi.
...
> **Tip**
> If many students show a gray dot inside their green ring, have them open Mission HydroSci as they arrive so the downloads run while you start class, instead of everyone downloading at the same moment when you say "launch."

## Solving problems with the Devices view

| The student says… | Look at… | What it usually means | What to do |
|---|---|---|---|
| "It's stuck on the loading screen" or "It won't launch." | The dot inside their green ring, and Storage. | Blue outline: the unit is still downloading. Gray: the download stopped or never started. | Have the student stay on the Mission HydroSci page until it says **Ready to play**. MHS retries a stopped download on its own; the page shows what it is doing and when it will retry. If the bar is red, free up space first. |
...
| "I'm on a different Chromebook today." | A new row under their name, with a gray dot inside the green ring until the unit downloads. | Nothing is wrong. The new device downloads its own copy of the unit; progress follows the account. | Have them open MHS a few minutes early so the download finishes before you start. |
...
|---|---|
| Whether a student's device is ready for the unit they are playing | The dot inside the green ring: blue is ready, gray is not downloaded |
