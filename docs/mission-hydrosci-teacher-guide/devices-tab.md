...
| **Unit 1 – Unit 5** | One mark per unit. A dot shows whether the unit is downloaded on this device, with a green ring around the dot on the student's current unit. A green check circle marks a unit the student has completed. An amber dot means the device reported a problem downloading that unit. See *Reading the unit dots* below. |
...
2. Scan for **amber** dots. Each one is a download the device reported trouble with. Then look at the dot inside each student's green ring on the device they use (the row with the most recent Last Seen). Gray means the unit is not on it yet; the download starts on its own when they open MHS, and takes a few minutes on school Wi-Fi.
...
> **Tip**
> If several students show gray inside the ring on the device they use, have them open Mission HydroSci as they arrive so the downloads run while you start class, instead of everyone downloading at the same moment when you say "launch."

## Solving problems with the Devices view

| The student says… | Look at… | What it usually means | What to do |
|---|---|---|---|
| "It's stuck on the loading screen" or "It won't launch." | The dot inside their green ring on the device in front of them, and Storage. | Amber: the download had a problem (hover for which). Blue outline: still downloading. Gray: not on this device yet. | Have the student stay on the Mission HydroSci page until it says **Ready to play**. MHS retries a stopped download on its own; the page shows what it is doing and when it will retry. If the bar is red, free up space first. |
