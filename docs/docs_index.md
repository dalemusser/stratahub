...
- [Members Report — Resolving Identity from Hex IDs](resource-identification/members-report.md) - The authorized crosswalk that maps de-identified hex IDs back to names, orgs, groups, and logins; its Identity in export selector also produces a de-identified (hex-only) roster
...
- [Overview and Guide Index](member-status-api/README.md) - Start here: how the survey links, the de-identified ids, the API, Survey Events, and the Surveys tab work together, and which guide is for whom
...
- [Plan](member-status-api/plan.md) - Design, decisions, and task-by-task implementation record; §8 specifies the Survey Events log (first data viewer); §9 the browser (CORS) calls
