...
**Mechanism, observed 2026-09-18 (supersedes the hypothesis below).** Reproduced on the dev workspace by blocking the log service host in the browser at launch: the game POSTs its first event, the retry ten seconds later, and then never sends again for the session, blocked or not, while writing every event to its store; a reload does not recover. The captured store shows why. The cached queue's first entry is an empty object, `{}`, and the send loop reads an empty head entry as "queue empty" and exits, every time, forever. The queue holds the caller's dictionary object rather than a copy; the session's first event (a debug-menu state event fired before identity is set, the same one that goes out without a user id) was emptied by its caller after it was handed in. Normally that first send is answered within a fraction of a second and the entry is dequeued before the clear; it bites only when the first send fails at the network level. One blip at launch, and the profile never logs again until its site data is cleared. Details, the reproduction recipe, and the specimen are in the handoff bundle (`mhs-updates/gamelogger-cache-overflow-091626/02-reproduction-and-specimen.md`). The fix needs two additions to the bundle's drop-in: copy the dictionary on enqueue, and skip empty entries when loading the cache.
