...
- `POST …/run/{testId}/questionnaire` — the post-play answers (a normal form post; no answer is required, an empty form is refused).
...
5. Docs: `docs/mhs-device-test/admin-guide.md` and `tester-guide.md` (given to schools as its GitHub link; how to enable it, the URL to give a school, what the school sees, how to read a run, the id marker); refresh the feature `README.md` and `issues/README.md` (DOC-1, DOC-2); note the marker in the stratalog and stratasave READMEs.
