...
**Task 3 — Member Status API feature.**
`internal/app/features/memberstatusapi/`: `handler.go` (deps: DB, settings store, users store, memberstatus store, ratelimiter, logger), `auth.go` (decode body once, extract `key`, load the workspace key, constant-time compare, throttle), `routes.go` (`MountRoutes(r, h)` registering `POST /api/member-status`, `POST /api/member-status/ping`), `json.go` (tiny `writeJSON`/`writeError`), `types.go`, `handler_test.go` (httptest: no key configured, wrong key, throttle, bad JSON, bad user_id, unknown user, apex/no workspace, happy paths, idempotency, monotonic state, unknown entity stored + flagged). Bootstrap: CSRF skip middleware before `routes.go:252`; maintenance exemption; mount outside the `RequireWorkspace` group (handler returns JSON 400 itself when `IDFromRequest` is nil).

**Task 4 — Dashboard config + data.**
`mhs_member_status.json` + embed directive; `internal/app/system/memberstatuscfg` (`Load`/`Items`/`Resolve`); dashboard types; `buildProgressRows` integration; org-timezone formatting; unit test for config loading and name matching (case-insensitive, aliases, unknown → `name:` key).
