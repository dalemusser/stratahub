...
**Task 4 — Dashboard config + data.** (Config file and `memberstatuscfg` package already delivered under Task 3.)
Remaining: dashboard types; `buildProgressRows` integration (`memberstatus.ListByUserIDs`, matched by `item.id` with the folded-name fallback); org-timezone formatting.
