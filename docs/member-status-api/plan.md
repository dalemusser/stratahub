...
`go build ./... && go vet ./... && make test-safe`; `make css-prod` (new Tailwind classes in templates); run locally, exercise with curl against `localhost:8080` (single-workspace mode) and check the tab in light/dark; deploy via `stratahub_update/aws_update.sh`; set the key on the production workspace; send Abt the provider guide + key out of band; confirm with a `ping` and one test event against a test member. The local verification is done (Tasks 0–6 and the viewers framework are on `main`); the production steps, with the exact commands and expected responses, are the checklist in [rollout-todo.md](rollout-todo.md). **Status 2026-09-07:** deployed 2026-08-29; keys set on Dev MHS and MHS; 15/15 end-to-end checks on Dev MHS via `scripts/member-status-api-check.sh` (2026-09-06); hand-off sent to Abt 2026-09-07 (MHS host, key, analyst login, test student). Open: Survey Tracking + ABT-deidentified on the four MHS resources (being set up separately), and Abt's implementation. Work is paused until Abt reports back; tick this task when their first real events land.
