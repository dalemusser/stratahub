...
RUN CGO_ENABLED=0 GOOS=linux go build -ldflags="-s -w" -o /app/stratahub ./cmd/stratahub
...
COPY --from=builder /app/stratahub .

# Copy resources (templates, assets, static)
