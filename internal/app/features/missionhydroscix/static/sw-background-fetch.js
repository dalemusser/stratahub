// sw-background-fetch.js — Background Fetch handlers for the Mission HydroSci X service worker.

const MHS_CHANNEL_NAME = 'missionhydroscix-delivery';

// Fetch ID format: "missionhydroscix-{unitId}-v{version}" e.g., "missionhydroscix-unit1-v1.0.0"
// This encodes everything we need to cache responses without fetching the manifest.

function parseFetchId(fetchId) {
  // "missionhydroscix-unit1-v1.0.0" -> { unitId: "unit1", version: "1.0.0" }
  var match = fetchId.match(/^missionhydroscix-(.+)-v(\d+\.\d+\.\d+)$/);
  if (match) {
    return { unitId: match[1], version: match[2] };
  }
  // Legacy format: "missionhydroscix-unit1"
  return { unitId: fetchId.replace('missionhydroscix-', ''), version: null };
}
...
async function startBackgroundFetch(unitId, version, files, cdnBaseUrl, title) {
  var fetchId = 'missionhydroscix-' + unitId + '-v' + version;
...
async function handleBackgroundFetchSuccess(event) {
  ...
  if (!version) {
    ...
    try {
      var manifestResponse = await fetch('/missionhydroscix/api/manifest');
  ...
  for (var i = 0; i < records.length; i++) {
    ...
    try {
      ...
      // We need: /missionhydroscix/content/unit1/v1.0.0/Build/unit1.loader.js
      ...
      if (idx !== -1) {
        ...
        var cacheKey = '/missionhydroscix/content/' + relativePath;
...
function handleBackgroundFetchClick(event) {
  event.waitUntil(
    clients.openWindow('/missionhydroscix/units')
  );
}
