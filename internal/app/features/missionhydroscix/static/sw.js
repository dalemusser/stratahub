// sw.js — Main service worker entry point for Mission HydroSci X content delivery.
// This file is concatenated with sw-cache.js and sw-background-fetch.js
// by the Go handler before being served at /missionhydroscix-sw.js.
    ...
    caches.keys().then(function(names) {
      return Promise.all(
        names.filter(function(name) {
          return name.startsWith('missionhydroscix-app-shell-') && name !== APP_SHELL_CACHE;
        }).map(function(name) {
...
self.addEventListener('fetch', function(event) {
  ...
  // /missionhydroscix/content/* — cache-first, fallback to CDN redirect
  if (path.startsWith('/missionhydroscix/content/')) {
  ...
  // /missionhydroscix/units, /missionhydroscix/play/* — cache-first for offline support
  if (path === '/missionhydroscix/units' || path.startsWith('/missionhydroscix/play/')) {
...
async function networkFirstWithOffline(request) {
  try {
    ...
    return new Response(
      ...
      '<a href="/missionhydroscix/units">Go to Units</a></body></html>',
