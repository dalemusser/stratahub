// sw-cache.js — Cache utility functions for the Mission HydroSci X service worker.

const APP_SHELL_CACHE = 'missionhydroscix-app-shell-v1';
const UNIT_CACHE_PREFIX = 'missionhydroscix-unit-';

// App shell resources to pre-cache on install.
const APP_SHELL_URLS = [
  '/missionhydroscix/units',
 ...
 * e.g., 'missionhydroscix-unit-unit1-v1.0.0'
 */
 ...
 * e.g., "unit1/Build/unit1.data.br" -> "/missionhydroscix/content/unit1/Build/unit1.data.br"
 */
function cacheKeyFromPath(filePath) {
  return '/missionhydroscix/content/' + filePath;
}
...
async function cleanStaleCaches(manifest) {
  ...
  for (const cacheName of allCaches) {
    if (cacheName.startsWith(UNIT_CACHE_PREFIX) || cacheName.startsWith('missionhydroscix-app-shell-')) {
