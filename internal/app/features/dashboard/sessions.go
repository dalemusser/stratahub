...
func SessionsRoutes(h *SessionsHandler, sessionMgr *auth.SessionManager) http.Handler {
	r := chi.NewRouter()
	r.Use(sessionMgr.RequireRole("superadmin", "admin"))
...
func (h *SessionsHandler) listSessions(w http.ResponseWriter, r *http.Request) {
	...
	vm := SessionsListVM{
		BaseVM:       viewdata.NewBaseVM(r, h.db, "Active Sessions", "/dashboard"),
		Sessions:     sessionVMs,
...
func (h *SessionsHandler) terminateSession(w http.ResponseWriter, r *http.Request) {
	...
	// Close the session
	if err := h.sessions.Close(r.Context(), sess.Token, sessions.EndReasonAdminTerminated); err != nil {
