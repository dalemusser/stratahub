...
//	    OrgID    string `validate:"required" label:"Organization"`
//	}
//
...
func getValidator() *validate.Validator {
	validatorOnce.Do(func() {
		...
		// resourcetype: validates against models.ResourceTypes
		customValidator.RegisterRuleFunc("resourcetype", func(value any) bool {
			if s, ok := value.(string); ok {
				return IsValidResourceType(s)
			}
			return false
		}, "resourcetype")
...
//   - authmethod: field must be a valid auth method (internal, google, etc.)
//   - resourcetype: field must be a valid resource type from models.ResourceTypes
...
//	    Role   string `validate:"required,oneof=admin leader member" label:"Role"`
...
func formatMessage(label, rule, param string) string {
	...
	case "resourcetype":
		return label + " must be a valid resource type."
	case "httpurl":
...
//
// RFC 5322 allows single-label domains (domains without dots), which are useful for:
//   - localhost: local development and testing (e.g., user@localhost)
//   - Intranet hostnames: internal mail servers (e.g., admin@mailserver)
//   - Windows/AD domains: Active Directory short names (e.g., user@CORP)
//   - Container environments: Docker/Kubernetes service names (e.g., app@mailservice)
//
// Note: Single-label domains are valid per RFC but won't work for internet email
// delivery. They are only useful for internal systems, dev environments, and testing
// where the hostname resolves locally.
func IsValidEmail(email string) bool {
	...
	// This handles edge cases like quoted strings, comments, and single-label domains.
