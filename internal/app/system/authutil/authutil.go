...
// for user creation and editing forms across all user roles.
package authutil
...
var (
	...
	// AuthReturnIDMethods are auth methods that require an auth_return_id
	AuthReturnIDMethods = map[string]bool{
		"clever":    true,
		"classlink": true,
		"schoology": true,
	}
...
// RequiresAuthReturnID returns true if the given auth method requires an auth_return_id.
func RequiresAuthReturnID(method string) bool {
	return AuthReturnIDMethods[method]
}
...
type AuthResult struct {
	...
	AuthReturnID     *string // Optional auth_return_id (set if provided)
...
var (
	...
	ErrAuthReturnIDRequired = errors.New("Auth Return ID is required for this authentication method.")
...
func ValidateAndResolve(input AuthInput) (*AuthResult, error) {
	...
	if EmailIsLogin(input.Method) {
		// For email/google/microsoft: email is required and becomes login_id
		if input.Email == "" {
	...
	// Validate auth_return_id for methods that require it
	if RequiresAuthReturnID(input.Method) && input.AuthReturnID == "" {
		return nil, ErrAuthReturnIDRequired
	}
	...
	if input.AuthReturnID != "" {
		result.AuthReturnID = &input.AuthReturnID
	}
...
// RequiresAuthReturnIDMethod returns true if the current auth method requires auth_return_id.
// This is a template helper method.
func (d TemplateData) RequiresAuthReturnIDMethod() bool {
	return RequiresAuthReturnID(d.Auth)
}
