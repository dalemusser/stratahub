...
| Feature | Purpose |
|---------|---------|
| **mhsdashboard** | MHS-specific dashboard views. Devices tab (2026-09): rows are devices (newest first per student); the unit cells' background is the student's progress (green completed, striped purple current, from grading: a unit is finished when every point is passed or flagged, current is the grader's `CurrentUnit`), the dot is the device's download state (blue, blue ring, small gray, amber badge = reported download problem); every state has a non-color cue. Teacher-facing text + figures in `docs/mission-hydrosci-teacher-guide/`; `devices_fixture_test.go` renders the figure page. Progress tab review pop-up (2026-09-21): a flagged grade carries `reasons: [{code, variables}]` from mhsgrader; `reasoncodes.go` fills the spec's Instructor Message template for (point, code) from the embedded `mhs_reason_codes.json` (generated from `mhsgrading/grading-logic` by mhsgrader's `cmd/mhsreasoncodes`) and shows the point's Teacher Guidance; a point started again after a finished attempt shows the finished grade with a note |
| **mhsbuilds** | Build/activity tracking for MHS platform. Ceremony versions (`end/vX.Y.Z/` on the CDN, `Kind` ceremony, id `end`) are registered by the S3 sync and chosen per collection (`MHSCollection.Ceremony`) on the manual/edit forms |
| **missionhydrosci** | MHS-specific configuration and management. The launcher posts device status (`/api/device-status`) on page load, on each download completion, and on `error`/`stalled` (first at once, then ≥60 s apart per unit) — the dashboard's Devices tab reads it. **End-of-game ceremony** (2026-09): `GET /missionhydrosci/ceremony` is a bare host page that mounts the `mhs-gameplay-end` bundle from the resolved collection's ceremony version through the same-origin content path (`/missionhydrosci/content/end/v<ver>/`, SW cache or CDN redirect) and the bundle's embed API; `GET /api/ea-scores` serves the student's EA checkpoint scores from the grader DB (`ceremony.go`: latest finished attempt per point, `status` pending/ready, staff `?user_id=` within `viewscope`); `POST /api/ceremony/viewed` records the marks on `mhs_user_progress`; the play page's `mhsEndGame` lands on the ceremony when the collection has one, the units page offers **Watch your ceremony** once complete. Plan: `docs/mission-hydrosci/mhs-end-ceremony-plan.md` |
